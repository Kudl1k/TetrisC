...
bool colision(Tetrino *block,Gameboard *board);
...
bool colision(Tetrino *block,Gameboard *board){
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            if ((block->shape[i][j] == 1 && board->grid[blk.y][blk.x + 1] == 3) || (block->shape[i][j] == 1 && board->grid[blk.y][blk.x - 1] == 3))
            {
                return true;
            }
            blk.x++;
